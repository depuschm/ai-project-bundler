...
- **Skips dependencies properly.** `node_modules`, `.git`, `bin`, `obj` and friends are pruned during the walk, so they're never read. On a test tree with a small `node_modules`, that's the difference between a five-second run producing 702 files and a 55ms run producing the 2 you wanted — and a real `node_modules` is far larger.
